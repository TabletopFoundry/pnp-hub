    ...
    <div className="section-shell py-12">
      <div className="paper-panel rounded-[2.2rem] border border-[rgba(77,57,36,0.08)] p-7 sm:p-8">
        ...
        <div className="mt-4 flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="rounded-[1.5rem] bg-white/70 px-4 py-3 text-sm text-[rgba(58,43,31,0.74)]">Signed in as Paper Sparrow Studio</div>
        </div>
      ...
      {submitted ? (
        <div className="mt-6 rounded-[1.6rem] border border-[rgba(54,90,76,0.16)] bg-[rgba(54,90,76,0.1)] px-5 py-4 text-sm text-[var(--ink)]">
          Draft saved to SQLite. Refresh the page or scroll to “My games” to see the new submission.
        </div>
      ) : null}
      {errorMessage ? (
        <div className="mt-6 rounded-[1.6rem] border border-[rgba(181,110,79,0.16)] bg-[rgba(181,110,79,0.1)] px-5 py-4 text-sm text-[var(--ink)]">
          {errorMessage}
        </div>
      ) : null}
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <UploadForm />
        <div className="paper-panel rounded-[1.9rem] border border-[rgba(77,57,36,0.08)] p-6">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            <div className="rounded-[1.5rem] bg-[rgba(54,90,76,0.1)] p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-[rgba(58,43,31,0.72)]">75% of gross revenue goes back to the designer on every paid sale and included catalog pool.</p>
            </div>
            <div className="rounded-[1.5rem] bg-[rgba(216,165,65,0.12)] p-5">
              ...
              <p className="mt-2 text-sm leading-6 text-[rgba(58,43,31,0.72)]">25% funds payment processing, editorial review, optimizer tooling, and community surfaces.</p>
            </div>
          </div>
          <div className="mt-5 rounded-[1.5rem] bg-white/75 p-5 text-sm leading-6 text-[rgba(58,43,31,0.78)]">
      ...
      <div className="mt-8 paper-panel rounded-[2rem] border border-[rgba(77,57,36,0.08)] p-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <span className="rounded-full bg-[rgba(216,165,65,0.12)] px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--ink)]">{dashboard.games.length} tracked titles</span>
        </div>
        {dashboard.games.length ? (
          <div className="mt-5 overflow-x-auto">
            <table className="min-w-full border-separate border-spacing-y-3 text-sm text-[rgba(58,43,31,0.78)]">
              <thead>
                ...
                {dashboard.games.map((game) => (
                  <tr key={game.slug} className="rounded-[1.4rem] bg-white/80">
                    <td className="rounded-l-[1.4rem] px-4 py-4">
                      ...
                      <p className="mt-1 text-xs uppercase tracking-[0.16em] text-[rgba(58,43,31,0.56)]">{game.category}</p>
                    </td>
                    <td className="px-4 py-4">
                      <span className={`rounded-full px-3 py-2 text-xs font-semibold uppercase tracking-[0.18em] ${game.status === 'published' ? 'bg-[rgba(54,90,76,0.12)] text-[var(--forest)]' : 'bg-[rgba(181,110,79,0.12)] text-[var(--terracotta)]'}`}>
                        {game.status}
                      </span>
                    </td>
